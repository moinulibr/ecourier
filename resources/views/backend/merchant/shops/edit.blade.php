                      ...
                      @csrf
                        ...
                        <div class="col-xs-12 col-sm-12 col-lg-12">
                            <div class="form-group">
                                 <input type="text" name="shop_name" value="{{ $shop->shop_name }}" placeholder="Enter Shop Name" class="form-control">
                            </div>
                         </div>
                                      ...
                                      @foreach($districts as $district)
                                        <optgroup label="{{ $district->name }}">
                                          @foreach($district->area as $value)
                                            <option {{ $value->id == $shop->area_id ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->area_name }}</option>
                                          @endforeach
                                        </optgroup>
                                      @endforeach
