          ...
          <div class="col-lg-12">
              <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <hr>
                    <form action="{{ route('merchant.order.store') }}" method="post">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                  <div class="form-group">
                                    <select name="merchant_shop_id" id="merchant_shop_id" class="form-control merchant_shop_id_class">
                                      @foreach($merchant_shopes as $merchant_shop)
                                      <option {{ old('merchant_shop_id') == $merchant_shop->id?'selected':''}} value="{{ $merchant_shop->id }}">{{ $merchant_shop->shop_name }}</option>
                                      @endforeach
                                    </select>
                                    <div style='color:red; padding: 0 5px;'>{{($errors->has('merchant_shop_id'))?($errors->first('merchant_shop_id')):''}}</div>
                                  </div>
                            </div>
                            <input type="hidden" name="merchant_id" class="form-control merchant_id_class" value="{{ Auth::guard('merchant')->user()->id }}">
                            ...
                            <div class="col-xs-12 col-md-4 col-lg-4">
                                <div class="row">
                                   ...
                                   {{--   <div class="col-xs-12 col-sm-12 col-lg-12">
                                    ...
                                    <div class="col-xs-12 col-sm-12 col-lg-12">
                                      <div class="form-group">
                                            <input type="text" name="collect_amount" id="collect_amount_id" class="form-control collect_amount_class" placeholder="Collection Amount">
                                      </div>
                                    </div>
                                    <div class="col-xs-12 col-sm-12 col-lg-12">
                                          <div class="form-group">
...
<div id="getmerchantshop" data-url="{{ route('merchant.getMerchantShopAjax')}}"></div>
@push('js')
  <script>
...
<input type="hidden" id="ifOrderExisting" data-url="{{route('merchant.ifOrderExisting')}}" >
<input type="hidden" id="makingDeliveryCharge" data-url="{{route('merchant.makingDeliveryCharge')}}" >
<script>
  //============default Details of Delivery Charge section ==========
    ...
    $(document).on('click, change','.parcel_category_id_class,.service_type_id_class,.parcel_type_id_class,.merchant_shop_id_class,.weight_id_class,.area_id_class',function(e){
        e.preventDefault();
         //var merchantId = getValueFromSelectOption('merchant_id_class');
         var merchantId = $('.merchant_id_class').val();
