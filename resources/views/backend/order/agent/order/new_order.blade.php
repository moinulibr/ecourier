                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xs-12 col-sm-12 col-lg-12">
                                        <div class="form-group">
                                            ...
                                            <span id="collectAmountShowErrorMessage" style="color:red;"></span>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            <select name="parcel_amount_payment_type_id" id="parcel_amount_payment_type_id" required class="parcel_amount_payment_type_id_class form-control ">
                                                    @foreach($parcelAmountPaymentTypies as $value)
                                                        <option  {{ old('parcel_amount_payment_type_id') == $weight->id?'selected':'' }} value="{{ $value->id }}">{{ $value->name }}</option>
                                                    @endforeach
                                                </select>
    ...
    $(document).on('click, change','.parcel_category_id_class,.service_type_id_class,.parcel_type_id_class,.merchant_id_class,.merchant_shop_id_class,.weight_id_class,.area_id_class',function(e){
        e.preventDefault();
            ...
            $('#collectable_from_customer_id').text(collectAmount);
            $('#collectable_amount_from_customer').val(collectAmount);
    ...
    $(document).on('change keyup keydown','.parcel_amount_payment_type_id_class,.charge,.collect_amount_class',function(){
        ...
        //console.log("cod = "+ cod +", Collect Amunt = " + collectAmount + ", Del Charge = "+ delivery);
    });

    function calculationFormulaByParcelAmountPaymentTypeId(checkByVariable,deliveryCharge,codCharge,CollectAmount)
    {
        $('#collectAmountShowErrorMessage').text('');
        var delCharge   = nanCheck(deliveryCharge);
        var cod         = nanCheck(codCharge);
        var colAmount   = nanCheck(CollectAmount);
        var totalCharge = delCharge + cod;
        
        var collectAmountFromCustomer = 0;
        var collectAmountFromMerchant = 0;
        var payableToMerchantOrClient = 0;

        if(checkByVariable == 1)
        {
        ...
        else if(checkByVariable == 2)
        {
            ...
            if(colAmount == 0)
            {
                ...
                collectAmountFromCustomer = colAmount;
                collectAmountFromMerchant = 0;
                payableToMerchantOrClient = colAmount - totalCharge;
            }
             collectAmountRemoveErrorMessage();
        }
        ...
        else if(checkByVariable == 3)
        {
            ...
            if(colAmount == 0)
            {
                ...
                collectAmountFromCustomer = colAmount;
                collectAmountFromMerchant = totalCharge;
                payableToMerchantOrClient = colAmount;
            }
             collectAmountRemoveErrorMessage();
        }
        else if(checkByVariable == 4)
        {
            ...
            if(colAmount == 0)
            {
                ...
                collectAmountFromCustomer = colAmount;
                collectAmountFromMerchant = delCharge; 
                payableToMerchantOrClient = colAmount;
            }
             collectAmountRemoveErrorMessage();
        }
        else if(checkByVariable == 5)
        {
            ...
            if(colAmount == 0)
            {
                ...
                collectAmountShowErrorMessage(colAmount, delCharge);
                collectAmountFromCustomer = colAmount;
                collectAmountFromMerchant = cod; 
                payableToMerchantOrClient = 0;
            }
        ...
        else if(checkByVariable == 6)
        {
            ...
            $('#collectAmountShowErrorMessage').text("");
            $('#deliveryChargeErrorMessage').text("");
        ...
        $('#total_charge_id').text(totalCharge);
        $('#totalChargeId').val(totalCharge);

        $('#collectable_from_customer_id').text(collectAmountFromCustomer);
        $('#collectable_amount_from_customer').val(collectAmountFromCustomer);

        $('#collectable_from_merchant_id').text(collectAmountFromMerchant);
        $('#collectable_amount_from_merchant').val(collectAmountFromMerchant);

        $('#total_payable_amount_id').text(payableToMerchantOrClient);
        $('#total_payable_amount').val(payableToMerchantOrClient);
    }

    function collectAmountShowErrorMessage(colAmount, delivery)
    {
        if(colAmount != delivery)
        {
            $('#collectAmountShowErrorMessage').text("Please check amount");
            $('#deliveryChargeErrorMessage').text("Please check amount");
        }else{
            $('#collectAmountShowErrorMessage').text("");
            $('#deliveryChargeErrorMessage').text("");
        }
    }
    function collectAmountRemoveErrorMessage()
    {
        $('#collectAmountShowErrorMessage').text("");
        $('#deliveryChargeErrorMessage').text("");
    }
    function CollectAmountZeroErrorMessageAndRemove(colAmount)
    {
        if(colAmount)
        {
            $('#collectAmountShowErrorMessage').text('');
        }else{
            $('#collectAmountShowErrorMessage').text('Please Fillout the field first');
        }
    }
    //nan check
    ...
    function paymentOptionShoHide(checkByVariable)
    {
