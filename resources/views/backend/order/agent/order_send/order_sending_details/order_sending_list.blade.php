    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('agent.pdfDownloadManpowerOrderAssingedList')}}" method="GET">
                    <div class="row" >
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Sender Name  </label>
                                <select name="sender_id" id="sender_id" class="sender_id_class form-control select2">
                                    <option value="">Select One Sender</option>
                                    @foreach($senders as $sender)
                                    <option value="{{$sender->id}}">{{$sender->name}}</option>
                                    @endforeach
                                </select>
                                <div style='color:red; padding: 0 5px;'>{{($errors->has('sender_id'))?($errors->first('sender_id')):''}}</div>
                            </div>
                        </div>
...
@section('ajaxdropdown')   
    ...
    <div id="parcelListAjaxList" data-url="{{ route('agent.sending.parcelListAjaxList')}}"></div>
    {{--Making Cart--}}
    <script>
        $(document).ready(function(){
            ...
            vKey = 86,
            cKey = 67;
            $(document).on('keyup change','.sender_id_class,.from_date_id_class,.to_date_id_class',function(e)
            {
                if (e.keyCode == ctrlKey || e.keyCode == cmdKey) ctrlDown = true;
                if (ctrlDown && (e.keyCode == vKey || e.keyCode == cKey)) return false;
                    var url = $('#parcelListAjaxList').data("url");
                    var sender_id           =  getValueFromSelectOption('sender_id_class');
                    ...
                    $.ajax({
                        url: url,
                        type: "GET",
                        data: {sender_id:sender_id,from_date:from_date,to_date:to_date},
    ...
    {{---Print----}}
    <div id="parcelListViewAjaxList" data-url="{{ route('agent.sending.parcelListViewAjaxList')}}"></div>
    <script>
            // Customer Copy Print
            $(document).on('click','.viewByDate',function(e){
            e.preventDefault();
                var url = $('#parcelListViewAjaxList').data('url');
                var sender_id           =  getValueFromSelectOption('sender_id_class');
                var from_date           =  getValueFromSelectOption('from_date_id_class');
                var to_date             =  getValueFromSelectOption('to_date_id_class');
    ...
    {{----Pagination by ajax-----}}
    <input type="hidden" id="getDataByPagination" data-url="{{route('agent.manpowerAssignedOrder')}}" >
    <script>
