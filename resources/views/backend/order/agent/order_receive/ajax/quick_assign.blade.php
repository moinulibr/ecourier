...
<tr>
    ...
    <td>
        <span id="set-{{ $item['receiving_from_branch'] }}">{{ $item['receiving_from_branch'] }}</span>
    </td>
    <td>
        <span id="set-{{ $item['destination_branch'] }}">{{ $item['destination_branch'] }}</span>
    </td>
    <td>
        <a href="#" class="clickForGetId"  data-url="{{ route('agent.receive_quickAssignParcelRemoveSingleCart')}}" data-id="{{ $item['order_session_id'] }}">
